...
RegisterServerEvent('ctake:server:withdraw', function(type, amount)
    ...
    if player and player.money then
        ...
        if parsedAmount and player.money >= parsedAmount then
            if addPlayerMoney(type, parsedAmount) then
                player.money = player.money - parsedAmount
                savePlayerData(player, src)
            end
        end
    end
...
QBCore.Functions.CreateUseableItem(Config.CasinoItem, function(source)
    TriggerClientEvent('ctake:client:open', source)
end)
